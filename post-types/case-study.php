	...
	register_post_type(
		...
		array(
			'labels'                => array(
				...
				'singular_name'         => __( 'Case Studies', 'tekbyte-task-1' ),
...
function tekbyte_task_1_updated_messages( $messages ) {
	...
	$messages['case-study'] = array(
		...
		9  => sprintf( __( 'Case Studies scheduled for: <strong>%1$s</strong>. <a target="_blank" href="%2$s">Preview Case Studies</a>', 'tekbyte-task-1' ), date_i18n( __( 'M j, Y @ G:i', 'tekbyte-task-1' ), strtotime( $post->post_date ) ), esc_url( $permalink ) ),
		/* translators: %s: post permalink */
...
add_filter( 'post_updated_messages', 'tekbyte_task_1_updated_messages' );

/**
 * Sets the bulk post updated messages for the `case_study` post type.
 *
 * @param  array $bulk_messages Arrays of messages, each keyed by the corresponding post type. Messages are
 *                              keyed with 'updated', 'locked', 'deleted', 'trashed', and 'untrashed'.
 * @param  int[] $bulk_counts   Array of item counts for each message, used to build internationalized strings.
 * @return array Bulk messages for the `case_study` post type.
 */
function tekbyte_task_1_bulk_updated_messages( $bulk_messages, $bulk_counts ) {
	global $post;
...
add_filter( 'bulk_post_updated_messages', 'tekbyte_task_1_bulk_updated_messages', 10, 2 );
